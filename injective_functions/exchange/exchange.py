...
class InjectiveExchange:
    ...
    async def get_subaccount_deposits(self, subaccount_idx: int,  denoms: str = None) -> Dict:
        try:
            ...
            subaccount_id = await self.chain_client.address.get_subaccount_id(subaccount_idx)
            deposits = await self.chain_client.client.fetch_subaccount_deposits(subaccount_id=subaccount_id)["deposits"]
            ...
            if len(denoms) > 0:
                for denom in denoms:
                    if denom in deposits:
                        human_readable_deposits[denom] = {
                            "available_balance": str(deposits[denom]["availableBalance"] / denom_decimals[denom]),
                            "total_balance": str(deposits[denom]["totalBalance"] / denom_decimals[denom]) 
                        } 
                    else:
                        human_readable_deposits[denom] = {
                ...
                for (denom, deposit) in deposits.items():
                    human_readable_deposits[deposit] = {
                        "available_balance": str(deposit["availableBalance"] / denom_decimals[denom]),
                        "total_balance": str(deposit["totalBalance"] / denom_decimals[denom]) 
                    }
    ...
    async def launch_instant_perp_market(self, 
                                         ticker: str, 
                                         quote_denom: str, 
                                         oracle_base: str, 
                                         oracle_quote: str,
    ...
    async def opt_out_trade_earn_rewards(self) -> Dict: 
        await self.chain_client.init_client()
        msg = self.chain_client.composer.msg_rewards_opt_out(sender=self.chain_client.address.to_acc_bech32())
